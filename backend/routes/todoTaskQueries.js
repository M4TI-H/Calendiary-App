...
router.get("/fetch_todo_lists", async (req, res) => {
  try{
...
router.post("/add_todo_list", async (req, res) => {
  try {
    const authHeader  = req.headers["authorization"];
    if (!authHeader ) {
      return res.status(401).json({ status: "Access token missing." });
...
router.delete("/remove_list/:list_id", async (req, res) => {
  try{
...
router.get("/fetch_tasks/:id", async (req, res) => {
  try{
    const authHeader  = req.headers["authorization"];
    if (!authHeader ) {
      return res.status(401).json({ status: "Access token missing." });
...
router.get("/fetch_todays_tasks", async (req, res) => {
  try{
    const authHeader  = req.headers["authorization"];
    if (!authHeader ) {
      return res.status(401).json({ status: "Access token missing." });
    ...
    dateNow.toLocaleString("en-GB", {day: "2-digit", month: "short",  year: "numeric"}).replace(",", "").toString();

    const values = [decodedData.email, dateNow];
    const [data] = await db.query("SELECT todo_id, description, due_date, status FROM user, todo_tasks WHERE user.id = user_id AND user.email = ? AND due_date = ?", values);
