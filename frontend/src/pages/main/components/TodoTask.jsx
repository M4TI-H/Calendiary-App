...
export default function TodoTask({id, content, due_date, onDelete}) {
  ...
  return(
    <VStack align="flex-end">
      <Flex w="22rem" minH="3rem" maxH="auto" h="100%" pl={isDoubleClicked ? "0" : "3"} pr={isDoubleClicked ? "0" : "1"} py="1" flexDir="row" align="center" borderRadius="lg" boxShadow="md" 
      borderWidth={isDoubleClicked ? "2px" : "1px"} borderColor={isDoubleClicked ? "#248277" : "#CED4DA"} bg={isTaskDone ? "#CED4DA" : "#F8F9FA"}>
        {!isDoubleClicked &&
        ...
        {isDoubleClicked ?
          <Flex w="100%" align="baseline" justify="space-between" p="0">
            <Input defaultValue={content} onChange={e => setEditedText(e.target.value)} border="none" _focusVisible="false" maxLength="100"/>
          ...
          :
          <Text maxW="15rem" onDoubleClick={textDoubleClick} ml="3" mr="2" decoration={isTaskDone ? "line-through" : "none"} 
          color={isTaskDone ? "#ADB5BD" : "#2b2d42"} fontWeight="semibold" _hover={{cursor: "default"}}>
            {displayedText}</Text>
          }
        {!isDoubleClicked && (
          isTaskDone && (
            <IconButton h="2rem" w="2rem" onClick={deleteTask} bg="none" _hover={{ bg: "rgba(193, 18, 31, 0.5)" }} ml="auto" borderRadius="lg"> 
